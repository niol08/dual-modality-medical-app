...
def _pick_eog_raw(raw: mne.io.BaseRaw) -> mne.io.BaseRaw:
    ...
    if not eog_channels:
        # fallback: pick first channel if available, otherwise leave raw unchanged
        if raw.ch_names:
...
def _segment_windows(data: np.ndarray, sfreq: float, win_s: float = 5.0, step_s: float = 1.0, max_windows=None):
    ...
    if T < win:
        # pad time axis with zeros so we can create one full window
        pad = np.zeros((C, win - T), dtype=data.dtype)
...
def run_eog_detector(uploaded_file,
    ...
    try:
        ...
        ext = Path(tmp_edf).suffix.lower()
        raw = None
        # support CSV/TXT/NPY uploads by building an MNE RawArray
        if ext in ('.csv', '.txt', '.dat', '.npy'):
            try:
                if ext == '.npy':
                    arr = np.load(tmp_edf)
                else:
                    df = pd.read_csv(tmp_edf, header=None, comment='#', sep=None, engine='python')
                    # coerce to numeric and drop empty columns
                    df = df.apply(pd.to_numeric, errors='coerce')
                    df = df.dropna(axis=1, how='all')
                    if df.empty:
                        raise RuntimeError('No numeric data found in uploaded file')
                    arr = df.values
                ...
                if arr.ndim == 1:
                    ...
                    # common: rows = samples, cols = channels -> transpose
                    arr = arr.T

                sfreq = 256.0
                ch_names = [f"EOG{i}" for i in range(arr.shape[0])]
                info = mne.create_info(ch_names=ch_names, sfreq=sfreq, ch_types=['eog'] * arr.shape[0])
                raw = mne.io.RawArray(arr.astype(np.float32), info)
            except Exception:
                raw = None
